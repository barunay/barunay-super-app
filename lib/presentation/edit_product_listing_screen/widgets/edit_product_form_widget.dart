...
class _EditProductFormWidgetState extends State<EditProductFormWidget> {
  ...
  void initState() {
    super.initState();
    _currentCategories =
        widget.categories.where((cat) => cat['parent_id'] == null).toList();

    // Initialize category path if a category is already selected
    if (widget.selectedCategoryId != null) {
      _initializeCategoryPath();
    }
  }

  void _initializeCategoryPath() {
    ...
    if (selectedCategory.isNotEmpty) {
      _categoryPath = [selectedCategory['slug']];
    }
  }
  ...
  void _goBackInCategoryPath() {
    if (_categoryPath.isNotEmpty) {
      ...
      if (_categoryPath.isEmpty) {
        ...
        // This is simplified - in a full implementation, you'd track the parent IDs
        _loadSubcategories(_categoryPath.last);
      }
    }
  }
